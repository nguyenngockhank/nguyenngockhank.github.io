...
_Software systems should separate the startup process, when the application objects are
constructed and the dependencies are “wired” together, from the runtime logic that takes
over after startup._
The startup process is a _concern_ that any application must address. It is the first _con-
cern_ that we will examine in this chapter. The _separation of concerns_ is one of the oldest
...
method is doing more than one thing, so we are breaking the _Single Responsibility Principle_
in a small way.
...
know the global context? Can we _ever_ really know the right object to use here? Is it even
...
the global setup _strategy_ (if there is one) is _scattered_ across the application, with little
modularity and often significant duplication.

If we are _diligent_ about building well-formed and robust systems, we should never let
little, _convenient_ idioms lead to modularity breakdown. The startup process of object con-
...
pattern to give the application control of _when_ to build the LineItems, but keep the details
...
to the purpose, thereby supporting the _Single Responsibility Principle._ In the context of
...
mechanism will usually be either the “main” routine or a special-purpose _container._
...
constructor arguments (or both) that are used to _inject_ the dependencies. During the con-
...
_want_ such a road through their town?

It is a myth that we can get systems “right the first time.” Instead, we should imple-
ment only today’s _stories,_ then refactor and expand the system to implement new stories
...
ning? Certainly, _it_ can’t grow incrementally from simple to complex _,_ can it?
...
thereby imposed unnecessary barriers to organic growth. Consider an _Entity Bean_ for a
...
I haven’t shown the corresponding _LocalHome_ interface, essentially a factory used to
...
tence strategy across many objects. We use the term _cross-cutting concerns_ for concerns like
these. Again, the persistence framework might be modular and our domain logic, in isola-
tion, might be modular. The problem is the fine-grained _intersection_ of these domains.

In fact, the way the EJB architecture handled persistence, security, and transactions _,_
“anticipated” _aspect-oriented programming_ (AOP),^7 which is a general-purpose approach
to restoring modularity for cross-cutting concerns.

In AOP, modular constructs called _aspects_ specify which points in the system should
...
_patterns_ thereof) should be persisted and then delegate the persistence tasks to your persis-
...
We defined an interface Bank, which will be _wrapped_ by the proxy, and a _Plain-Old
Java Object_ (POJO), BankImpl, that implements the business logic. (We will revisit POJOs
shortly.)
...
to implement aspects in pure Java.^12 In Spring, you write your business logic as _Plain-Old
Java Objects_. POJOs are purely focused on their domain. They have no dependencies on
...
ture concerns at the code level, then it is possible to truly _test drive_ your architecture. You
...
demand. It is not necessary to do a _Big Design Up Front_^18 (BDUF). In fact, BDUF is even
...
Although software has its own _physics_ ,^20 it is economically feasible to make radical
change, _if_ the structure of the software separates its concerns effectively.
...
_simple_ at each level of abstraction and scope.
...
kill when they aren’t really needed. A good API should largely _disappear_ from view most
...
18. Not to be confused with the good practice of up-front design, BDUF is the practice of designing _everything_ up front before
...
become obsessed with various _strongly hyped_ standards and lose focus on implementing
...
ware, there has been renewed interest recently in creating _Domain-Specific Languages_
